...
public class BindViewUtil {
    ...
    public static void bindHeaderInf(ImageView avatar, TextView username, TextView postTime, TextView postDevice, TextView repostCount, TextView commentCount, TextView likeCount, Context mContext, Weibo weibo) {
        Glide.with(mContext).load(weibo.user.avatar_large).bitmapTransform(new CropCircleTransformation(mContext)).placeholder(R.mipmap.ic_person_white_24dp).into(avatar);
    ...
    public static void bindContent(TextView content, Context mContext, String text, OnWeiboContentListener listener) {
        content.setText(WeiboContentKeyUtil.getWeiBoContent(text, mContext, content, listener));
    }
